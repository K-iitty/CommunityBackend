...
public class ComplexBusinessController {
    ...
    public Result addVehicleForOwner(@Parameter(description = "业主ID") @PathVariable Long ownerId, 
        ...
        if (result) {
            // 发布实时同步消息
            try {
                redisMessageService.publishAdminChange("CREATE", "Vehicle", vehicle.getId(), vehicle);
                redisMessageService.publishNotification("owner", "VEHICLE_ADDED", "车辆添加", 
                    "管理员为您添加了车辆：" + vehicle.getPlateNumber(), ownerId);
                redisMessageService.publishNotification("property", "VEHICLE_ADDED", "车辆添加", 
                    "管理员添加了车辆：" + vehicle.getPlateNumber(), null);
            } catch (Exception e) {
    ...
    public Result linkHouseToOwner(@Parameter(description = "房屋ID") @PathVariable Long houseId, 
        ...
        if (result) {
            // 发布实时同步消息
            try {
                redisMessageService.publishAdminChange("UPDATE", "HouseOwner", houseId, null);
                redisMessageService.publishNotification("owner", "HOUSE_LINKED", "房屋关联", 
                    "管理员为您关联了房屋", ownerId);
                redisMessageService.publishNotification("property", "HOUSE_LINKED", "房屋关联", 
                    "管理员关联了房屋和业主", null);
            } catch (Exception e) {
                System.err.println("发布房屋关联实时消息失败: " + e.getMessage());
            }
    ...
    public Result assignIssueToStaff(@Parameter(description = "问题ID") @PathVariable Long issueId, 
        ...
        if (result) {
            // 发布实时同步消息
            try {
                redisMessageService.publishAdminChange("UPDATE", "OwnerIssue", issueId, null);
                redisMessageService.publishNotification("owner", "ISSUE_ASSIGNED", "问题已分配", 
                    "您的问题已分配给物业人员处理", null);
                redisMessageService.publishNotification("property", "ISSUE_ASSIGNED", "问题分配", 
                    "管理员分配了新的问题给您处理", staffId);
            } catch (Exception e) {
                System.err.println("发布问题分配实时消息失败: " + e.getMessage());
            }
    ...
    public Result addHouseOwnerRelation(@Parameter(description = "业主ID") @RequestParam Long ownerId,
        ...
        if (result) {
            // 发布实时同步消息
            try {
                redisMessageService.publishAdminChange("CREATE", "HouseOwner", null, null);
                redisMessageService.publishNotification("owner", "HOUSE_OWNER_RELATION", "房屋关联", 
                    "管理员为您关联了新的房屋", ownerId);
                redisMessageService.publishNotification("property", "HOUSE_OWNER_RELATION", "房屋业主关联", 
                    "管理员创建了新的房屋业主关联", null);
            } catch (Exception e) {
                System.err.println("发布房屋业主关联实时消息失败: " + e.getMessage());
            }
