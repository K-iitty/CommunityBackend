...
import com.community.admin.service.OwnerIssueService;
...
//        }
     ...
     * @param bestContactTime 最佳联系时间
     * @param urgencyLevel 紧急程度
     * @param assignedStaffId 指派处理人员ID
     * @param assignedDepartmentId 指派部门ID
     * @param assignedRemark 指派备注
    ...
    public Map<String, Object> update(
            ...
            @Parameter(description = "最佳联系时间") @RequestParam(required = false) String bestContactTime,
            @Parameter(description = "紧急程度") @RequestParam(required = false) String urgencyLevel,
            @Parameter(description = "指派处理人员ID") @RequestParam(required = false) Long assignedStaffId,
            @Parameter(description = "指派部门ID") @RequestParam(required = false) Long assignedDepartmentId,
            @Parameter(description = "指派备注") @RequestParam(required = false) String assignedRemark,
            @Parameter(description = "问题状态") @RequestParam(required = false) String issueStatus,
            @Parameter(description = "工单状态") @RequestParam(required = false) String workStatus,
            @Parameter(description = "内部备注") @RequestParam(required = false) String internalRemark,
            @Parameter(description = "问题图片文件（可选）") @RequestParam(value = "issueImages", required = false) MultipartFile[] issueImages,
            @Parameter(description = "问题描述图片文件（可选）") @RequestParam(value = "additionalImages", required = false) MultipartFile[] additionalImages) {
        ...
        // 更新业主问题信息对象（只更新非null的字段）
        if (communityId != null) existingIssue.setCommunityId(communityId);
        if (ownerId != null) existingIssue.setOwnerId(ownerId);
        if (houseId != null) existingIssue.setHouseId(houseId);
        if (issueTitle != null) existingIssue.setIssueTitle(issueTitle);
        if (issueContent != null) existingIssue.setIssueContent(issueContent);
        if (issueType != null) existingIssue.setIssueType(issueType);
        if (subType != null) existingIssue.setSubType(subType);
        if (locationType != null) existingIssue.setLocationType(locationType);
        if (specificLocation != null) existingIssue.setSpecificLocation(specificLocation);
        if (contactName != null) existingIssue.setContactName(contactName);
        if (contactPhone != null) existingIssue.setContactPhone(contactPhone);
        if (bestContactTime != null) existingIssue.setBestContactTime(bestContactTime);
        if (urgencyLevel != null) existingIssue.setUrgencyLevel(urgencyLevel);
        if (internalRemark != null) existingIssue.setInternalRemark(internalRemark);
        
        // 处理指派相关字段
        if (assignedStaffId != null) {
            existingIssue.setAssignedStaffId(assignedStaffId);
            // 如果指派了人员，自动设置指派时间
            existingIssue.setAssignedTime(java.time.LocalDateTime.now());
        }
        if (assignedDepartmentId != null) existingIssue.setAssignedDepartmentId(assignedDepartmentId);
        if (assignedRemark != null) existingIssue.setAssignedRemark(assignedRemark);
        if (issueStatus != null) existingIssue.setIssueStatus(issueStatus);
        if (workStatus != null) existingIssue.setWorkStatus(workStatus);
...
//        }
    ...
    @Operation(summary = "指派问题处理人员", description = "为指定问题指派处理人员")
    @ApiOperationSupport(order = 5, author = "开发团队")
    @SecurityRequirement(name = "Authorization")
    public Result assignStaff(
            @Parameter(description = "业主问题ID") @PathVariable Long id,
            @Parameter(description = "指派处理人员ID", required = true) @RequestParam Long assignedStaffId,
            @Parameter(description = "指派部门ID") @RequestParam(required = false) Long assignedDepartmentId,
            @Parameter(description = "指派备注") @RequestParam(required = false) String assignedRemark) {
        
        OwnerIssue issue = ownerIssueService.getById(id);
        if (issue == null) {
            return Result.error("问题信息不存在");
        }
        
        // 更新指派信息
        issue.setAssignedStaffId(assignedStaffId);
        issue.setAssignedDepartmentId(assignedDepartmentId);
        issue.setAssignedRemark(assignedRemark);
        issue.setAssignedTime(java.time.LocalDateTime.now());
        
        // 更新问题状态和工单状态
        if ("待处理".equals(issue.getIssueStatus())) {
            issue.setIssueStatus("处理中");
        }
        if ("未分配".equals(issue.getWorkStatus()) || issue.getWorkStatus() == null) {
            issue.setWorkStatus("已分配");
        }
        
        boolean result = ownerIssueService.updateById(issue);
        if (result) {
            // 发布实时同步消息
            try {
                redisMessageService.publishAdminChange("UPDATE", "OwnerIssue", id, issue);
                redisMessageService.publishNotification("property", "ISSUE_ASSIGN", "问题指派", 
                    "您有新的问题需要处理：" + issue.getIssueTitle(), assignedStaffId);
            } catch (Exception e) {
                System.err.println("发布问题指派实时消息失败: " + e.getMessage());
            }
    ...
    public Result delete(@Parameter(name = "id", description = "业主问题ID", required = true, example = "1") @PathVariable Long id) {
        ...
        if (result) {
            // 发布实时同步消息
            try {
                redisMessageService.publishAdminChange("DELETE", "OwnerIssue", id, null);
                if (issue != null) {
                    redisMessageService.publishNotification("owner", "ISSUE_DELETE", "问题反馈删除", 
                        "您的问题反馈已被删除：" + issue.getIssueTitle(), issue.getOwnerId());
                    redisMessageService.publishNotification("property", "ISSUE_DELETE", "问题反馈删除", 
                        "问题反馈已删除：" + issue.getIssueTitle(), null);
                }
            } catch (Exception e) {
                System.err.println("发布问题删除实时消息失败: " + e.getMessage());
            }
