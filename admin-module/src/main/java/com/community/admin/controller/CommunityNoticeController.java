...
public class CommunityNoticeController {
    ...
    public Map<String, Object> add(
        ...
        if (result) {
            // 发布实时同步消息
            try {
                redisMessageService.publishAdminChange("CREATE", "CommunityNotice", communityNotice.getId(), communityNotice);
                redisMessageService.publishNotification("owner", "NEW_NOTICE", "新公告发布", 
                    "新公告：" + communityNotice.getTitle(), null);
                redisMessageService.publishNotification("property", "NEW_NOTICE", "新公告发布", 
                    "新公告：" + communityNotice.getTitle(), null);
            } catch (Exception e) {
                System.err.println("发布公告创建实时消息失败: " + e.getMessage());
            }
    ...
    public Map<String, Object> update(
        ...
        if (result) {
            // 发布实时同步消息
            try {
                redisMessageService.publishAdminChange("UPDATE", "CommunityNotice", existingNotice.getId(), existingNotice);
                redisMessageService.publishNotification("owner", "NOTICE_UPDATE", "公告更新", 
                    "公告已更新：" + existingNotice.getTitle(), null);
                redisMessageService.publishNotification("property", "NOTICE_UPDATE", "公告更新", 
                    "公告已更新：" + existingNotice.getTitle(), null);
            } catch (Exception e) {
                System.err.println("发布公告更新实时消息失败: " + e.getMessage());
            }
    ...
    public Result delete(@Parameter(name = "id", description = "社区公告ID", required = true, example = "1") @PathVariable Long id) {
        // 获取公告信息用于通知
        CommunityNotice notice = communityNoticeService.getById(id);
        
        boolean result = communityNoticeService.removeById(id);
        if (result) {
            // 发布实时同步消息
            try {
                redisMessageService.publishAdminChange("DELETE", "CommunityNotice", id, null);
                if (notice != null) {
                    redisMessageService.publishNotification("owner", "NOTICE_DELETE", "公告删除", 
                        "公告已删除：" + notice.getTitle(), null);
                    redisMessageService.publishNotification("property", "NOTICE_DELETE", "公告删除", 
                        "公告已删除：" + notice.getTitle(), null);
                }
            } catch (Exception e) {
                System.err.println("发布公告删除实时消息失败: " + e.getMessage());
            }
